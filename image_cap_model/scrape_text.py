...
# inputs a comic panel and outputs rectangles
# where text boxes are thought to exist
...
for index in tqdm(range(1, N_COMICS + 1)):
    ...
    if (abs(height - EXP_HEIGHT) > DIMENSION_THRESHOLD or
        abs(width - EXP_WIDTH) > DIMENSION_THRESHOLD):
        continue
    ...
    panels = [cv2.resize(image[panel_dim[i][0]:panel_dim[i][1], 
                         panel_dim[i][2]:panel_dim[i][3]],
    ...
    for i in range(NUM_PANELS):
        ...
        for text_box in text_boxes:
            ...
            textbox_locations.append((x / 1.5, y / 1.5))
            # recall that we scaled earlier
    ...
    with open(os.path.join(text_dir, text_csv_name), mode='w') as text_csv:
        writer = csv.writer(text_csv, delimiter=',',
                quotechar='"', quoting=csv.QUOTE_MINIMAL)
        writer.writerow(text_per_panel)

    with open(os.path.join(loc_dir, location_csv_name), mode='w') as loc_csv:
        writer = csv.writer(loc_csv, delimiter=',', 
                quotechar='"', quoting=csv.QUOTE_MINIMAL)
        row = [str(i) for i in textbox_locations]
        writer.writerow(row)

    for panel_index, panel in enumerate(panels):
        panel_name = 'panel_{:04d}_{}.jpg'.format(index, panel_index)
        cv2.imwrite(os.path.join(panel_dir, panel_name), panel)
